...
**API 응답속도 개선(과목 삭제 : 92.83% || 과목 저장 : 50.52% )** : API 응답속도를 체크하던 중, 과목 저장 및 삭제 기능에서 과도한 응답 지연이 발생하는 것을 확인했습니다.  <br /> 로그 기반으로 분석한 결과 클라이언트에서 전달받은 데이터를 처리하는 과정에서 JPA의 영속성 컨텍스트 반영 및 다량의 쿼리 실행이 성능 저하의 주요 원인임을 파악했습니다. <br /> JPA의 설정을 조정하여 최적화를 시도했으나, 기대만큼 성능 개선이 이루어지지 않아 Native SQL을 직접 활용하여 개선했습니다.     <br />
<br />
**삭제 기능**: IN 조건을 사용한 단일 쿼리로 일괄 삭제 처리
<br />
**저장 기능**: Native SQL + JdbcTemplate.batchUpdate()를 이용한 JDBC 배치 처리 방식 적용
<br />

이러한 개선을 통해 저장 및 삭제 기능의 API 응답 속도가 각각 **50.52%, 92.83%** 개선되었습니다.
