...
**API 응답속도 개선(과목 삭제 : 92.83% || 과목 저장 : 50.52% )** : 기능들의 API 응답속도를 체크하던 중 저장 삭제 부분의 응답속도가 느리다는 것을 확인했습니다.   <br /> 로그를 통해 분석한 결과 클라이언트에서 받아온 데이터 확인, 데이터 저장, 삭제과정에서 발생하는 JPA 영속성 확인과 그 후에 발생하는 다량의 쿼리 처리로 인한 문제인 것으로 확인되었습니다.  <br /> JPA 를 사용해서 문제해결을 하려고 했지만 만족할만큼의 속도가 나오지 않아 Native SQL 을 사용하여 처리하였습니다. 저장의 경우 Nativce SQL + JDBC 배치 처리 방식을 사용하여 최적화 하였습니다.    <br />
